...
@section('content')
    ...
    <form class="repeater" id="add_expense" name="add_expense" role="form" method="POST" action="{{ url('admin/edit_expense') }}" autocomplete="off">
        @csrf
        <input type="hidden" id="expense_id" name="expense_id" value="{{ $expense->id }}">
        
        <div class="flex items-center justify-between mb-lg">
            <h3 class="text-2xl font-bold text-dark m-0">{{ __('frontend.edit_expense') }}</h3>
        ...
        <x-form-card title="{{ __('frontend.expense') }}">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-xl mb-xl pb-lg border-b border-gray-light">
                ...
                <div>
                    ...
                    <label class="block text-sm font-semibold text-gray-dark mb-xs">{{ __('frontend.billed_from') }} <span class="text-danger">*</span></label>
                ...
                <div class="space-y-md">
                    <div>
                        ...
                        <input type="text" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent" id="inv_no" name="inv_no" value="{{ $expense->invoice_no }}">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-dark mb-xs">{{ __('frontend.bill_date') }}: <span class="text-danger">*</span></label>
                        <input type="text" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent inc_Date" id="inv_date" name="inv_date" value="{{ date($date_format_laravel,strtotime($expense->inv_date)) }}">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-dark mb-xs">{{ __('frontend.bill_due_date') }}: <span class="text-danger">*</span></label>
                        <input type="text" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent due_Date" id="due_Date" name="due_Date" value="{{ date($date_format_laravel,strtotime($expense->due_date)) }}">
                    </div>
                </div>
            </div>
                ...
                <table class="w-full text-start text-sm text-dark tableInv" id="purchaseInvoice" data-repeater-list="group">
                    <thead class="bg-gray-50 border-b border-gray-light text-gray-dark uppercase text-xs dynamicRows">
                        <tr>
                            ...
                            <th width="10%" class="px-4 py-3 text-center hide with_tax">{{ __('frontend.tax') }}</th>
                                    ...
                                    <td class="p-2">
                                        ...
                                        <select class="w-full px-3 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent sel categories_ids" name="categories_ids" id="categories_ids" data-rule-required="true">
                                            ...
                                            @foreach($category as $cat)
                                                <option value="{{ $cat->id }}" @if(isset($item) && $item->category_id == $cat->id ) selected @endif>{{ $cat->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="p-2">
                                        <input type="text" class="w-full px-3 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent" id="description" name="description" value="{{ $item->description }}">
                                    </td>
                                    <td class="p-2">
                                        <input type="text" class="w-full px-3 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent text-center qty" id="qty" name="qty" data-rule-required="true" maxlength="10" onkeypress='return isNumber(event)' value="{{ $item->iteam_qty }}">
                                    </td>
                                    <td class="p-2">
                                        <input type="text" class="w-full px-3 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent text-center rate" onkeypress='return isFloatsNumberKey(event)' id="rate" name="rate" data-rule-required="true" maxlength="10" value="{{ $item->item_rate }}">
                                    </td>
                                    <td class="p-2">
                                        <input type="text" class="w-full px-3 py-2 border-transparent bg-transparent text-end font-bold amount" id="amount" name="amount" data-rule-required="true" readonly="" value="{{ $item->item_amount }}">
                                    </td>
                                    <td class="p-2 text-center">
                                        <button type="button" data-repeater-delete class="text-danger hover:text-red-700 bg-transparent border-none cursor-pointer p-2 btn_remove">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-xl">
                ...
                <div>
                    ...
                    <textarea class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent" id="note" name="note" rows="4">{{ $expense->remarks }}</textarea>
                </div>
                ...
                <div class="bg-gray-50 p-lg rounded-md border border-gray-light">
                    <table class="w-full mb-xs" id="tab_logic_total">
                        <tr>
                            ...
                            <td class="w-1/2">
                                <input type="text" name="subTotal" class="w-full bg-transparent border-none text-end font-bold text-lg expence-sub-total" id="subTotal" readonly="" value="{{ $expense->sub_total_amount ?? ''}}">
                            </td>
                        </tr>
                    </table>
                        ...
                        <tr>
                            <th class="text-start py-2">
                                <select id="tax" class="w-full px-3 py-1.5 border border-gray-light rounded-md text-sm tax" name="tax">
                                    ...
                                    @foreach($tax as $t)
                                        <option MyTax="{{ $t->per }}" value="{{ $t->id }}" @if(isset($expense) && $expense->tax_id == $t->id ) selected @endif>{{ $t->name.' '.$t->per.'%' }}</option>
                                    @endforeach
                                </select>
                            </th>
                            <td class="w-1/2">
                                <input type="text" name="taxVal" class="w-full bg-transparent border-none text-end font-bold text-lg expence-sub-total" id="taxVal" readonly="" value="{{ $expense->tax_amount ?? ''}}">
                            </td>
                        </tr>
                    </table>
                    ...
                    <table class="w-full border-t border-gray-300 mt-sm pt-sm" id="tab_logic_total">
                        <tr>
                            ...
                            <td class="w-1/2">
                                <input type="text" name="total" class="w-full bg-transparent border-none text-end font-bold text-2xl text-primary" id="grandTotal" readonly="" value="{{ $expense->total_amount ?? ''}}">
                            </td>
                        </tr>
                    </table>
                </div>

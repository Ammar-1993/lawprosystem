...
@section('content')
    ...
    <div class="flex items-center justify-between mb-lg">
        <h3 class="text-2xl font-bold text-dark m-0">{{ __('frontend.edit_case') }}</h3>
    ...
    <form method="post" name="add_case" id="add_case" action="{{ route('case-running.update', $case->id) }}">
        @csrf()
        @method('patch')
                ...
                <div>
                    ...
                    <select class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" name="client_name" id="client_name">
                        ...
                        @foreach ($client_list as $list)
                            <option value="{{ $list->id }}" {{ ($list->id == $case->advo_client_id) ? 'selected' : '' }}>{{ $list->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex items-center gap-md pt-md">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" id="test1" name="position" value="Petitioner" {{ (!empty($case) && $case->client_position=='Petitioner') ? 'checked' : '' }} class="text-primary focus:ring-primary border-gray-light">
                        <span class="ms-2 text-sm text-dark">{{ __('frontend.petitioner') }}</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" id="test2" name="position" value="Respondent" {{ (!empty($case) && $case->client_position=='Respondent') ? 'checked' : '' }} class="text-primary focus:ring-primary border-gray-light">
                        ...
                        <div data-repeater-item class="flex flex-wrap md:flex-nowrap items-end gap-md mb-md">
                            <div class="w-full md:w-5/12">
                                <label class="block text-sm font-semibold text-gray-dark mb-xs">
                                    <b class="position_name"></b>
                                </label>
                                <input type="text" id="party_name" name="party_name" data-rule-required="true" data-msg-required="Please enter name." class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $party->party_name }}">
                            </div>
                            <div class="w-full md:w-5/12">
                                <label class="block text-sm font-semibold text-gray-dark mb-xs">
                                    <b class="position_advo"></b>
                                </label>
                                <input type="text" id="party_advocate" name="party_advocate" data-rule-required="true" data-msg-required="{{ __('frontend.party_advocate') }}" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $party->party_advocate }}">
                            </div>
        ...
        <x-form-card title="{{ __('frontend.case_detail') }}">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-lg mb-lg">
                <div>
                    ...
                    <input type="text" value="{{ $case->case_number ?? '' }}" id="case_no" name="case_no" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent">
                </div>
                <div>
                    ...
                    <select class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" id="case_type" name="case_type" onchange="getCaseSubType(this.value);">
                        ...
                        @foreach ($caseTypes as $caseType)
                            <option value="{{ $caseType->id }}" {{ (!empty($case) && $case->case_types==$caseType->id) ? 'selected' : '' }}>{{ $caseType->case_type_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label for="case_sub_type" class="block text-sm font-semibold text-gray-dark mb-xs">{{ __('frontend.case_sub_type') }}</label>
                    <select class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" id="case_sub_type" name="case_sub_type">
                        @foreach($caseSubTypes as $caseSubType)
                            <option value="{{ $caseSubType->id }}" {{ (!empty($case) && $case->case_sub_type==$caseSubType->id) ? 'selected' : '' }}>{{ $caseSubType->case_type_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" id="case_status" name="case_status">
                        ...
                        @foreach ($caseStatuses as $caseStatus)
                            <option value="{{ $caseStatus->id }}" myvalue="{{ $caseStatus->case_status_name }}" {{ (!empty($case) && $case->case_status==$caseStatus->id) ? 'selected' : '' }}>{{ $caseStatus->case_status_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex items-center gap-md pt-md">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" id="test3" name="priority" value="High" {{ (!empty($case) && $case->priority=='High') ? 'checked' : '' }} class="text-primary focus:ring-primary border-gray-light">
                        <span class="ms-2 text-sm text-dark">{{ __('frontend.high') }}</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" id="test4" name="priority" value="Medium" {{ (!empty($case) && $case->priority=='Medium') ? 'checked' : '' }} class="text-primary focus:ring-primary border-gray-light">
                        <span class="ms-2 text-sm text-dark">{{ __('frontend.medium') }}</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" id="test5" name="priority" value="Low" {{ (!empty($case) && $case->priority=='Low') ? 'checked' : '' }} class="text-primary focus:ring-primary border-gray-light">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-lg mb-lg">
                <div>
                    ...
                    <input type="text" id="act" name="act" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $case->act ?? '' }}">
                </div>
                <div>
                    ...
                    <input type="text" id="filing_number" name="filing_number" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $case->filing_number }}">
                </div>
                <div>
                    ...
                    <input type="text" id="filing_date" name="filing_date" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent datetimepickerfilingdate" readonly="" value="{{ date($date_format_laravel,strtotime($case->filing_date)) }}">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-lg mb-lg">
                <div>
                    ...
                    <input type="text" id="registration_number" name="registration_number" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $case->registration_number }}">
                </div>
                <div>
                    ...
                    <input type="text" id="filiregistration_dateng_date" name="registration_date" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent datetimepickerregdate" readonly="" value="{{ date($date_format_laravel,strtotime($case->registration_date)) }}">
                </div>
                <div>
                    ...
                    <input type="text" id="next_date" name="next_date" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent datetimepickernextdate" readonly="" value="{{ date($date_format_laravel,strtotime($case->next_date)) }}">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-lg">
                <div class="md:col-span-1">
                    ...
                    <input type="text" id="cnr_number" name="cnr_number" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $case->cnr_number }}">
                </div>
                <div class="md:col-span-3">
                    <label for="description" class="block text-sm font-semibold text-gray-dark mb-xs">{{ __('frontend.description') }}</label>
                    <textarea id="description" name="description" rows="3" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent">{{ $case->description ?? '' }}</textarea>
                </div>
            </div>
        </x-form-card>
        ...
        <x-form-card title="{{ __('frontend.fir_details') }}">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-lg">
                <div class="md:col-span-3">
                    ...
                    <input type="text" id="police_station" name="police_station" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $case->police_station ?? '' }}">
                </div>
                <div class="md:col-span-1">
                    <label for="fir_number" class="block text-sm font-semibold text-gray-dark mb-xs">{{ __('frontend.fir_number') }}</label>
                    <input type="text" id="fir_number" name="fir_number" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $case->fir_number ?? '' }}">
                </div>
                <div class="md:col-span-1">
                    <label for="fir_date" class="block text-sm font-semibold text-gray-dark mb-xs">{{ __('frontend.fir_date') }}</label>
                    <input type="text" id="fir_date" name="fir_date" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent datetimepickerregdate" readonly="" value="@php if($case->fir_date!=null){@endphp {{date($date_format_laravel,strtotime($case->fir_date))}} @php } @endphp">
                </div>
            </div>
        </x-form-card>
        ...
        <x-form-card title="{{ __('frontend.court_detail') }}">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-lg mb-lg">
                <div>
                    ...
                    <input type="text" id="court_no" name="court_no" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $case->court_no ?? '' }}">
                </div>
                <div>
                    ...
                    <select class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" id="court_type" name="court_type" onchange="getCourt(this.value);">
                        ...
                        @foreach ($courtTypes as $courtType)
                            <option value="{{ $courtType->id }}" {{ (!empty($case) && $case->court_type==$courtType->id) ? 'selected' : '' }}>{{ $courtType->court_type_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" id="court_name" name="court_name">
                        @foreach($courts as $court)
                            <option value="{{ $court->id }}" {{ (!empty($case) && $case->court==$court->id) ? 'selected' : '' }}>{{ $court->court_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent select2" id="judge_type" name="judge_type">
                        ...
                        @foreach ($judges as $judge)
                            <option value="{{ $judge->id }}" {{ (!empty($case) && $case->judge_type==$judge->id) ? 'selected' : '' }}>{{ $judge->judge_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label for="judge_name" class="block text-sm font-semibold text-gray-dark mb-xs">{{ __('frontend.judge_type') }}</label>
                    <input type="text" id="judge_name" name="judge_name" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent" value="{{ $case->judge_name ?? '' }}">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1">
                <div>
                    ...
                    <textarea id="remarks" name="remarks" rows="2" class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent">{{ $case->remark ?? '' }}</textarea>
                </div>
            </div>
        </x-form-card>

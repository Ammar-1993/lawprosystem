...
@section('content')
    ...
    <form class="repeater" id="add_invoice" name="add_invoice" role="form" method="POST" action="{{url('admin/edit_invoice')}}" autocomplete="off">
        {{ csrf_field() }}

        <div class="flex items-center justify-between mb-lg">
            <h3 class="text-2xl font-bold text-dark m-0">{{__('frontend.edit_invoice')}}</h3>
        ...
        <x-form-card title="{{__('frontend.invoice')}}">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-lg mb-lg">
                ...
                <div>
                    ...
                    <select class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent client_id" name="client_id" id="client_id" onchange="getClientDetail(this.value);">
                        ...
                        @foreach ($client_list as $list)
                            <option value="{{ $list->id }}" {{ $list->id == $invoice->client_id ? 'selected' : '' }}>{{ $list->name }}</option>
                        @endforeach
                    </select>
                    <div class="show_vendor_detail mt-md text-sm text-gray-dark">{!! $client_detail !!}</div>
                </div>
                ...
                <div class="bg-gray-50 p-md rounded-md border border-gray-light">
                    ...
                    <div class="flex items-center justify-between mb-sm pb-sm border-b border-gray-light">
                        ...
                        <span class="font-bold text-dark">{{ $invoice->invoice_no ?? '' }}</span>
                    </div>
                    
                    <div class="flex items-center justify-between mb-sm pb-sm border-b border-gray-light">
                        <span class="text-sm font-semibold text-gray-dark">{{__('frontend.invoice_date')}} <span class="text-danger">*</span></span>
                        <input type="text" class="w-1/2 px-3 py-1 text-sm border border-gray-light rounded-md bg-white inc_Date text-end" id="inc_Date" name="inc_Date" readonly="" value="{{ date($date_format_laravel, strtotime($invoice->inv_date)) }}">
                    </div>
                    
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-semibold text-gray-dark">{{__('frontend.invoice_due_date')}} <span class="text-danger">*</span></span>
                        <input type="text" class="w-1/2 px-3 py-1 text-sm border border-gray-light rounded-md bg-white due_Date text-end" id="due_Date" name="due_Date" readonly="" value="{{ date($date_format_laravel, strtotime($invoice->due_date)) }}">
                    </div>
                </div>
            </div>
                                ...
                                <td class="p-2">
                                    <select class="w-full px-2 py-1 text-sm border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent sel services" name="services" id="services" data-rule-required="true">
                                        ...
                                        @foreach ($service_lists as $service)
                                            <option MyServiceAmount="{{ $service->amount }}" @if (isset($i) && $i->service_id == $service->id) selected @endif value="{{ $service->id }}">{{ $service->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td class="p-2">
                                    <input type="hidden" id="id" name="id" value="{{ $i->id ?? '' }}">
                                    <input type="text" class="w-full px-2 py-1 text-sm border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent" id="description" name="description" value="{{ $i->item_description ?? '' }}">
                                </td>
                                <td class="p-2">
                                    <input type="text" class="w-full px-2 py-1 text-sm text-center border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent qty" id="qty" name="qty" value="{{ $i->iteam_qty ?? '' }}" data-rule-required="true" maxlength="8" onkeypress='return isNumber(event)'>
                                </td>
                                <td class="p-2">
                                    <input readonly="" type="text" class="w-full px-2 py-1 text-sm text-end border border-gray-light rounded-md bg-gray-50 rate" onkeypress='return isFloatsNumberKey(event)' id="rate" name="rate" data-rule-required="true" maxlength="10" value="{{ $i->item_rate ?? '' }}">
                                </td>
                                <td class="p-2">
                                    <input type="text" class="w-full px-2 py-1 text-sm text-end border border-gray-light rounded-md bg-gray-50 amount" id="amount" name="amount" data-rule-required="true" readonly="" value="{{ $i->item_amount ?? '' }}">
                                </td>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-lg">
                <div>
                    ...
                    <textarea class="w-full px-4 py-2 border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent" id="note" name="note" rows="4">{{ $invoice->remarks ?? '' }}</textarea>
                ...
                <div class="bg-gray-50 p-md rounded-md border border-gray-light">
                    ...
                    <table class="w-full mb-sm" id="tab_logic_total">
                        <tr>
                            ...
                            <td class="text-end py-2 w-1/2">
                                <input value="{{ $invoice->sub_total_amount ?? '' }}" type="text" name="subTotal" class="w-full px-3 py-1 text-end border border-gray-light rounded-md bg-white expence-sub-total" id="subTotal" readonly="">
                            </td>
                        </tr>
                    </table>
                        ...
                        <tr>
                            <th class="text-start py-2">
                                <select id="tax" class="w-full px-2 py-1 text-sm border border-gray-light rounded-md focus:outline-none focus:ring-2 focus:ring-accent tax" name="tax">
                                    ...
                                    @foreach ($tax as $t)
                                        <option MyTax="{{ $t->per }}" value="{{ $t->id }}" {{ $invoice->tax_id == $t->id ? 'selected' : '' }}>{{ $t->name . ' ' . $t->per . '%' }}</option>
                                    @endforeach
                                </select>
                            </th>
                            <td class="text-end py-2 w-1/2 ps-2">
                                <input type="text" value="{{ $invoice->tax_amount ?? '' }}" name="taxVal" class="w-full px-3 py-1 text-end border border-gray-light rounded-md bg-white expence-sub-total" id="taxVal" readonly="">
                            </td>
                        </tr>
                    </table>
                    ...
                    <table class="w-full border-t border-gray-light pt-sm" id="tab_logic_total">
                        <tr>
                            ...
                            <td class="text-end py-2 w-1/2">
                                <input type="text" name="total" class="w-full px-3 py-1 text-end text-lg font-bold border-0 bg-transparent text-primary" id="grandTotal" readonly="" value="{{ $invoice->total_amount ?? '' }}">
                            </td>
                        </tr>
                    </table>
                </div>
